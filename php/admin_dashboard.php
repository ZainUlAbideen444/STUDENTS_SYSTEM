...
if(isset($_GET['del_student'])){
    $id = intval($_GET['del_student']);
    $check = mysqli_query($conn,"SELECT * FROM payments WHERE student_id=$id");
    if(mysqli_num_rows($check) > 0){
        echo "<script>alert('Cannot delete student. Payment records exist.');</script>";
    } else {
...
if(isset($_GET['del_course'])){
    $id = intval($_GET['del_course']);
    $check = mysqli_query($conn,"SELECT * FROM payments WHERE course_id=$id");
    if(mysqli_num_rows($check) > 0){
        echo "<script>alert('Cannot delete course. Payment records exist.');</script>";
    } else {
        mysqli_query($conn,"DELETE FROM courses WHERE id=$id");
    }
}

if(isset($_GET['del_payment'])) mysqli_query($conn,"DELETE FROM payments WHERE id=".$_GET['del_payment']);
...
<h3>Students</h3>
<table>
<tr><th>Name</th><th>Email</th><th>Phone</th><th>Course</th><th>Action</th></tr>
...
<td><?= $s['phone'] ?></td>
<td><?= $s['course_name'] ?></td>
<td><a href="?del_student=<?= $s['id'] ?>">Delete</a></td>
</tr>
<?php } ?>
</table>
...
<h3>Courses</h3>
<table>
<tr><th>Name</th><th>Fee</th><th>Duration</th><th>Action</th></tr>
<?php while($c=mysqli_fetch_assoc($courses)){ ?>
<tr>
<td><?= $c['course_name'] ?></td>
<td><?= $c['fee'] ?></td>
<td><?= $c['duration'] ?></td>
<td><a href="?del_course=<?= $c['id'] ?>">Delete</a></td>
</tr>
<?php } ?>
</table>
...
<h3>Payments</h3>
<table>
<tr><th>Student</th><th>Course</th><th>Amount</th><th>Method</th><th>Date</th><th>Action</th></tr>
...
<td><a href="?del_payment=<?= $p['id'] ?>">Delete</a></td>
</tr>
<?php } ?>
</table>
<?php } ?>
